...
**myorm_mysql** is a lightweight ORM written in **TypeScript** for **MySQL**.
...
Decorators are responsible for mapping classes, properties, and relationships directly to MySQL structures.
...
### @DataType
Explicitly defines the MySQL data type for the column using DBTypes.
...
var context = new Context(MySQLDBManager.Build("localhost", 5432, "test_db", "username", "password"));
...
```typescript
 let persons = await context.Persons.WhereAsString(`age > 30 or name like '%adriano%'`).ToListAsync();
```
...
The `MYSQLDBSet<T>.GetTypeMapping()` feature exposes metadata about an entity mapping, allowing you to safely build **dynamic SQL statements** based on entity definitions, column mappings, and relationships — without hardcoding table or column names.

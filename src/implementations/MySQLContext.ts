...
export default abstract class MySQLContext extends AbstractContext
{
    ...
    public GetMappedTypes()
    {
        ...
        {
            if((this as any)[prop].constructor == MySQLDBSet)
            {
...
export class JoinSelectable<T extends Object> implements IJoinSelectable<T>
{
    ...
    protected PrepareQuery(): AbstractSet<T> 
    {
        ...
        {           
            ...
            }else if (leftSideIsArray && !rightSideIsArray)
            {
               ...
               if(leftSideRelation)
               {
                   ...
                   query += ` JSON_CONTAINS(\`${leftSideTable}\`.\`${leftColumnName}\`, CAST(\`${rigthSideTable}\`.\`${rightColumnName}\` AS JSON))`;
               ...
               else
               {
                    ...
                    let areSameType =  this._context["_manager"]["CastToMySQLType"](elementType) == this._context["_manager"]["CastToMySQLType"](rightSideDBType);
                    let areSerial =  this._context["_manager"]["CastToMySQLType"](elementType) == DBTypes.AUTO_INCREMENT && this._context["_manager"]["CastToMySQLType"](rightSideDBType) == DBTypes.AUTO_INCREMENT;
            ...
            {  
               ...
               if(rightSideRelation)
               {
                   ...
                   query += ` JSON_CONTAINS(\`${rigthSideTable}\`.\`${rightColumnName}\`, CAST(\`${leftSideTable}\`.\`${leftColumnName}\` AS JSON))`;
               }
               else
               {
                    ...
                    let areSameType =  this._context["_manager"]["CastToMySQLType"](elementType) == this._context["_manager"]["CastToMySQLType"](leftSideDBType);
                    let areSerial =  this._context["_manager"]["CastToMySQLType"](elementType) == DBTypes.AUTO_INCREMENT && this._context["_manager"]["CastToMySQLType"](leftSideDBType) == DBTypes.AUTO_INCREMENT;
        ...
        {
            let set = this._context.Collection(type.Type as {new (...args: any[]) : Object})! as MySQLDBSet<Object>;
        ...
        MySQLSetHelper.InjectSQL<T>(selectedSideSet! as MySQLDBSet<T>, query);
        MySQLSetHelper.InjectWhere<T>(selectedSideSet! as MySQLDBSet<T>, where); 
    ...
    private Reset() : void
    {
        for(let i = 1; i < this._stack.length; i++)
        {
            ...
            let set = this._context.Collection(type as {new (...args: any[]) : Object})! as MySQLDBSet<Object>;
            set["Reset"]();
        }

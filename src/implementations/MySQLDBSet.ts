...
import MySQLFluentField from "./MySQLFluentField";
...
{ 
    ...
    protected AddObjectAsync(obj : T, cascade : boolean = true, relations : (keyof T)[] = [], visiteds : any[] = []): Promise<T> {
        ...
        return this.CreatePromisse(async () => 
        {
            ...
            if(key != undefined && insertResult && insertResult.insertId)
            {
                (obj as any)[key.Property] = insertResult.insertId;
            }
    ...
    UpdateSelectionAsync(): Promise<number> {
        ...
        return this.CreatePromisse(async() => 
        {
            ...
            let whereSrt = MySQLSetHelper.ExtractWhereData(this);
                            ...
                            if(relation)
                            {
                                ...
                                if(subRelation && (subRelation.Relation == RelationType.ONE_TO_MANY || subRelation?.Relation == RelationType.MANY_TO_MANY)){
                                    ...
                                    if(subDesingType == Array)
                                    {
                                        ...
                                        let queryAllpks = `(select JSON_ARRAYAGG(${pkColumn.Column}) from ${this._table} ${whereSrt})`
            
                                        let subUpdate = `update ${subTableName} set ${subColumnName.Column} =  JSON_MERGE_PRESERVE (${subColumnName.Column} , ${queryAllpks})  where \`${subPkColumn.Column}\` = ${this.CreateValueStatement(Type.CastType(subPkColumn.Type), Reflect.get(i as any, subPK))}`;
                        ...
                        if(relation)
                        {
                            ...
                            if(subRelation && (subRelation.Relation == RelationType.ONE_TO_MANY || subRelation?.Relation == RelationType.MANY_TO_MANY)){
                                ...
                                if(subDesingType == Array)
                                {
                                    ...
                                    let subUpdate = `update ${subTableName} set ${subColumnName.Column} =  JSON_MERGE_PRESERVE(${subColumnName.Column} , ${queryAllpks})  where \`${subPkColumn.Column}\` = ${this.CreateValueStatement(Type.CastType(subPkColumn.Type), Reflect.get(set[0].Value as any, subPK))}`;
    ...
    public async CountAsync(): Promise<number> {
        ...
        { 
            ...
            let whereSrt = MySQLSetHelper.ExtractWhereData(this);
            let sqlSrt = MySQLSetHelper.ExtractSQLData(this);
            ...
            return Number.parseInt(r[0]["count(*)"]);            
    ...
    public WhereField<U extends keyof T, R extends MySQLDBSet<T>>(field: U): IFluentField<T, U, R> {        
        
        this.ResetFilters();

        return new MySQLFluentField(this as any as R, field, false);
    }
    public AndField<U extends keyof T, R extends MySQLDBSet<T>>(field: U): IFluentField<T, U, R> {        
        return new MySQLFluentField(this as any as R, field, false);
    }
    public OrField<U extends keyof T, R extends MySQLDBSet<T>>(field: U): IFluentField<T, U, R> {        
        return new MySQLFluentField(this as any as R, field, true);
    }
    

    public WhereAsString<R extends MySQLDBSet<T>>(where : string) : R
    {
        this.ResetFilters();
    ...
    public LoadRelationOn<U extends keyof T, R extends MySQLDBSet<T>>(field: U): R {   
    ...
    private EvaluateStatement(pgStatement : IMySQLStatement)
    {
        ...
        if(Type.IsNumber(Type.CastType(typeName!.toString())) || Type.IsDate(Type.CastType(typeName!.toString())))
        {
            ...
            if(Type.IsDate(Type.CastType(typeName!.toString()))){                  
                    ...
                    {
                        pgStatement.Statement.Value = `'${dtStr}'`;
                    }
                    else
                    {
                        pgStatement.Statement.Value = `'${dtStr} ${dt.getHours()}:${dt.getMinutes()}'` ;
                    }
            }
        ...
        {
            if(pgStatement.Statement.Value && pgStatement.Statement.Value.endsWith("'"))
                pgStatement.Statement.Value = pgStatement.Statement.Value.substring(0, pgStatement.Statement.Value.length - 1);

            pgStatement.Statement.Value = StringUtils.ReplaceAll(pgStatement.Statement.Value, "'", "''");
        }

        return `\`${this._table}\`.\`${column}\` ${operation[0]} ${operation[1]}${pgStatement.Statement.Value}${operation[2]}`;
    }

    private EvaluateOrderBy(ordering : IMySQLOrdenation<T>)
    {
    ...
    private ResetFilters() : void
    {
        ...
        MySQLSetHelper.CleanORMData(this);
    }
        ...
        for(let row of r)
        {
            ...
            for(let map of this._maps)
            {
                ...
                if(!this._context.IsMapped(type!)){
                    ...
                    let columType = this._manager["CastToMySQLType"](Type.GetDesingTimeTypeName(this._type, map.Field)!);

                    if(Type.IsArray(columType) && v)
                        v = JSON.parse(v);
                    ...
                    if(v != undefined){
                        ...
                        if([DBTypes.INTEGER, DBTypes.LONG, DBTypes.AUTO_INCREMENT].includes(vType))
                    ...
                    if(Reflect.get(row, map.Column) == undefined)
                    {
                        if(!this._untrackeds)
                        {
                            ...
                            if(map.Type == "Array" && v && typeof v == 'string')
                                v = JSON.parse(v);
                    ...
                    if(includeType.length > 0)
                    {
                        ...
                        if(relation?.Relation == RelationType.MANY_TO_MANY || relation?.Relation == RelationType.ONE_TO_MANY)
                        {
                            ...
                            if(Type.IsArray(columType) && values)
...
interface IMySQLIncluding<T>
{
    Field : keyof T    
}

interface IMySQLLimiter
{
    Limit : number
}

interface IMySQLOffset
{
    OffSet : number
}
